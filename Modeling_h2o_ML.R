...
train_tbl <- recipe_obj %>% bake(newdata = train_readable_tbl)
test_tbl <- recipe_obj %>% bake(newdata = test_readable_tbl)
...
h2o.getModel("StackedEnsemble_BestOfFamily_0_AutoML_20180704_223910") %>%
...
glm_h2o <- h2o.loadModel("04_Modeling/h2o_models/GLM_grid_0_AutoML_20180704_223910_model_0")


# Making Predictions

stacked_ensemble_h2o <- h2o.loadModel("04_Modeling/h2o_models/StackedEnsemble_BestOfFamily_0_AutoML_20180704_223910")
...
data_transformed <- automl_models_h2o@leaderboard %>% as.tibble() %>%
...
h2o.auc(performance_h2o, valid = T, xval = T, train = T) #When Performance_h20 object is selected valid T etc may not work. It works on actual models

h2o.auc(stacked_ensemble_h2o, valid = T, xval = T, train = T)
...
performance_tbl <- performance_h2o %>% h2o.metric()%>%
  as.tibble() %>%
  glimpse()
...
performance_tbl %>% ggplot(aes(x = threshold)) +
  geom_line(aes(y = precision), size = 1, color = "blue") +
  geom_line(aes(y = recall), size = 1 , color = "red") +
  geom_vline(xintercept = h2o.find_threshold_by_max_metric(performance_h2o, "f1")) +
  theme_tq() +
  labs(title = "Precision vs Recall", y = "value")

# Performance for 3 classifiers ----

path <- "04_Modeling/h2o_models/DeepLearning_0_AutoML_20180704_223910"

#For deep learning model this function works( one model)
load_model_perf_metrics <- function(path, test_tbl){
...
#load_model_perf_metrics(path, test_tbl)

#If we want it to run for all 3 models

model_metrics_tbl <- fs::dir_info(path = "04_Modeling/h2o_models/") %>%
  select(path) %>%
  mutate(metrics = map(path, load_model_perf_metrics, test_tbl)) %>%
  unnest()

#We can do data manipulation and take only model name and convert to factor and auc also factor
...
model_metrics_tbl %>%
  ...
  labs(title = "ROC Curve",
       subtitle = "Performance of top 3 Models")
...
ranked_pred_tbl <- predictions_tbl %>%
  bind_cols(test_tbl) %>%
  select(predict:Yes, Attrition) %>%
  arrange(desc(Yes))

#In order to group them in yes batchwise use ntile function

calculated_gain_lift_tbl <- ranked_pred_tbl %>%
  ...
  mutate(
    ...
    pct_response = responses/sum(responses),
    gain = cumsum(pct_response),
...
#data_fraction is cum_pct_cases, lift is cum_lift, gain is cum_capture_rate
gain_transformed_tbl <- gain_lift_tbl %>% select(group, cumulative_data_fraction, cumulative_capture_rate, cumulative_lift)%>%
...
lift_transformed_tbl <- gain_lift_tbl %>% select(group, cumulative_data_fraction, cumulative_capture_rate, cumulative_lift)%>%
...
#get rowwise details with mutate and map for all model ids
