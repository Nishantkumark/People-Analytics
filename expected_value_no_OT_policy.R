...
total_ev_with_OT_tbl <- expected_val_with_OT_tbl %>% summarise(total_expected_attrition_cost_0 = sum(expected_attrition_cost))
...
test_without_OT_tbl <- test_tbl %>%
  mutate(OverTime = fct_recode(OverTime,"No" = "Yes")) #Wherever the OT is yes will be replaced by "No"
...
ev_without_OT_tbl <- predictions_without_OT_tbl %>% 
  ...
  mutate(
    cost_of_policy_change = case_when(OverTime_0 == "Yes" & OverTime_1 == "No" ~ avg_overtime_pct * attrition_cost, TRUE ~ 0)
  ) %>%
  mutate(
    expected_attrition_cost = Yes * (attrition_cost + cost_of_policy_change) + No * (cost_of_policy_change)
  )
